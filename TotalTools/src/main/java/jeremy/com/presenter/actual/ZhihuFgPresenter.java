...
import jeremy.com.R;
import jeremy.com.adapter.ZhihuListAdapter;
...
public class ZhihuFgPresenter extends BasePresenter<IZhihuFgView> {
    ...
    private void disPlayZhihuList(NewsTimeLine newsTimeLine, Context context, IZhihuFgView iZhihuFgView, RecyclerView recyclerView) {
        if (isLoadMore) {
            if (time == null) {
                adapter.updateLoadStatus(ZhihuListAdapter.LOAD_NONE);
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    ...
                    if (lastVisibleItem + 1 == layoutManager
                            .getItemCount()) {
                        adapter.updateLoadStatus(ZhihuListAdapter.LOAD_PULL_TO);
                        isLoadMore = true;
                        adapter.updateLoadStatus(ZhihuListAdapter.LOAD_MORE);
